...
bu da programımız zaten ekrana çok fazla yazdırma işlemi yapıyorsa gözden kaçabilir.
Böyle durumlarda ``assert`` ifadesini kullanabiliriz. Aslında ``assert`` ifadesi
aynı zamanda hata yükseltmenin kısa bir yoludur. Tabii ``assert`` ifadesini
